...
1. Extract `phase_correlate_shift(img1, img2)` shared with `NavigationCore`.
...
1. Extract shared `phase_correlate_shift()` with `MapStitcher`.
...
## Shared Algorithm Duplication

`MapStitcher._estimate_displacement()` and `NavigationCore._estimate_displacement()` duplicate phase-correlation logic. Extract:
...
This is a high-value, low-risk shared module if covered by existing path/localization/stitcher tests plus one direct unit test.
